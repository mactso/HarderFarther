	...
	public static ItemStack doGetLootStack(ServerLevel level, Vec3 origin) {
		...
		if (ci.lootItem instanceof ArmorItem) {
			  stack.enchant(Enchantments.ALL_DAMAGE_PROTECTION, enchantmentLevel);
				stack.setHoverName(new TranslatableComponent("item.harderfarther.ancient_armor.name"));
		}
		else
		...
		if (ci.lootItem instanceof BowItem) {
			  stack.enchant(Enchantments.POWER_ARROWS, 6);
				stack.setHoverName(new TranslatableComponent("item.harderfarther.ancient_bow.name"));
				Utility.setLore(stack,
						Component.Serializer.toJson(new TranslatableComponent("item.harderfarther.ancient_bow.lore")));
		}
		else
		if (ci.lootItem instanceof SwordItem) {
			...
			if (swordroll < 15) {
				...
				stack.setHoverName(new TranslatableComponent("item.harderfarther.ancient_sword.name"));
				Utility.setLore(stack,
						Component.Serializer.toJson(new TranslatableComponent("item.harderfarther.ancient_sword.lore")));
			} else if (swordroll < 25) {
				stack.enchant(Enchantments.SMITE, 6);
				stack.setHoverName(new TranslatableComponent("item.harderfarther.ancient_sword.name"));
				Utility.setLore(stack,
				Component.Serializer.toJson(new TranslatableComponent("item.harderfarther.ancient_smite_sword.lore")));
			} else {
				stack.enchant(Enchantments.BANE_OF_ARTHROPODS, 6);
				stack.setHoverName(new TranslatableComponent("item.harderfarther.ancient_sword.name"));
				Utility.setLore(stack,
				Component.Serializer.toJson(new TranslatableComponent("item.harderfarther.ancient_bane_sword.lore")));
			}
		}
		else
		if (ci.lootItem instanceof PickaxeItem) {
			  stack.enchant(Enchantments.BLOCK_EFFICIENCY, 6);
			  stack.setHoverName(new TranslatableComponent("item.harderfarther.ancient_pickaxe.name"));
		}
		else
		if (ci.lootItem instanceof AxeItem) {
			  stack.enchant(Enchantments.BLOCK_EFFICIENCY, 6);
				stack.setHoverName(new TranslatableComponent("item.harderfarther.ancient_axe.name"));
		}
		else
		if (ci.lootItem instanceof ShovelItem) {
			  stack.enchant(Enchantments.BLOCK_EFFICIENCY, 6);
				stack.setHoverName(new TranslatableComponent("item.harderfarther.ancient_shovel.name"));
		}
		else
